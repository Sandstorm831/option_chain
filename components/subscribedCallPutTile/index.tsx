export default function SubscribedCallPutTile({
  callPut,
  dataIdx,
  text_colr,
  subscribeByIdx,
  ...
  return (
    <div className="flex flex-col p-2 bg-gray-50 rounded-lg h-full justify-center mt-2 font-mono">
      <div className="flex justify-center font-bold text-xl">
        {dataIdx ? `${dataIdx[2]}${callPut}` : null}
      </div>
      <div className="flex justify-center text-sm">
        {dataIdx ? `Option Type : ${opt_type}` : null}
      </div>
      <div className="flex justify-center text-sm">
        {dataIdx ? `Option Strike : ${dataIdx[2]}` : null}
      </div>
      <div className="flex justify-center text-sm whitespace-pre-wrap">
        <div>Option Price : </div>
        <div className={`text-sm ${text_colr}`}>
          {dataIdx ? dataIdx[price_ind] : null}
        </div>
      </div>
      <div className="flex justify-center text-sm whitespace-pre-wrap">
        <div>Price Change : </div>
        <div className={`text-sm ${text_colr}`}>
          {dataIdx ? dataIdx[change_ind] : null}%
        </div>
      </div>
      <div
